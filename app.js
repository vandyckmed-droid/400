...
(() => {
  ...
  /* ---------- the score ----------
     ...
     score pane) agree to the last digit. The window is fixed at 9–1 and the
     measure is the score, with two choices:
       vol      divide the return by its own volatility
       resid    use the return net of the market instead of the return
     and one reading of that number, `display`: its 'value', its integer
     'rank' across the whole universe (1 = best), or its percentile 'pct'
     across the whole universe (100 = best). Display never changes the order,
     only the number shown. */
  const SCORE_DEFAULTS = { vol: false, resid: false, display: 'pct' };
  const WINDOW = '9–1';
  ...
  const scoreKey = (s = state.score) => adjustKey(s);
  const SCALE = 1000000;                                         // as build.py's SCALE
  const quant = (v) => Math.floor(v * SCALE + 0.5);              // as build.py's quant
  /* What the window measures under the adjustments, from its legs: r the
     return, v its volatility, e the return net of the market, w the residual
     volatility. Null where the name has no legs. */
  function measure(legs, s = state.score) {
    const { r, v, e, w } = legs;
    if (r == null) return null;
    return s.resid ? (s.vol ? e / w : e) : (s.vol ? r / v : r);
  }
  /* The score from a name's legs: the measure itself, as the fixed-point
     integer the ladder is built from (`q`) and as a number (`s`). */
  function scoreFrom(legs, s = state.score) {
    const x = measure(legs, s);
    return x == null ? { q: null, s: null } : { q: quant(x), s: quant(x) / SCALE };
  }
  /* Rank against a ladder — the members' scores as ascending fixed-point ints
     — as 1 + the number strictly better, so ties share the better position. */
  function rankIn(ladder, q) {
    let lo = 0, hi = ladder.length;
    while (lo < hi) { const m = (lo + hi) >> 1; if (ladder[m] <= q) lo = m + 1; else hi = m; }
    return 1 + ladder.length - lo;
  }
  const pctOf = (rank, n) => (n > 1 ? 100 * (n - rank) / (n - 1) : 100);
  /* Score every row under `s`: {symbol: {q, s, rank, pct}}, plus the ladder
     and member count the ranks came from. */
  function scoreAll(s = state.score) {
    const by = {};
    for (const r of state.rows) by[r.symbol] = scoreFrom(r.legs, s);
    const ladder = Object.values(by).filter((x) => x.q != null).map((x) => x.q).sort((a, b) => a - b);
    for (const x of Object.values(by)) {
      x.rank = x.q == null ? null : rankIn(ladder, x.q);
  ...
  /* The score is a return, so it reads as a percentage — unless the volatility
     adjustment has made it a ratio, which has no unit. */
  const FMT = {
    value: (v) => (v == null ? '—' : state.score.vol
  ...
  /* One line naming the score as built: "9–1 · ÷ volatility · net of market". */
  const scoreSummary = (s = state.score) => [
    WINDOW, s.vol ? '÷ volatility' : '', s.resid ? 'net of market' : '',
  ].filter(Boolean).join(' · ');
  ...
  function showSettings() {
    ...
    $('method').innerHTML = [
      [`${WINDOW} return`, 'Total return on dividend- and split-adjusted closes over the nine months '
      ...
      ['Score', `That number is the score: ${s.vol ? 'a return per unit of risk' : 'a return'}, `
        + 'compared with no one and adjusted by nothing else.'],
  ...
  function sparkline(symbol) {
    ...
    if (scores) {
      ...
      const at = (i) => (scores[i] == null ? '—' : FMT[d](d === 'value' ? scores[i] / SCALE : d === 'rank' ? ranks[i] : pcts[i]));
  ...
  /* ---------- events ---------- */
  function wire() {
    for (const [id, attr] of [['display-seg', 'display']]) {
      ...
      <details class="sect">
        ...
        <div class="body">
          <dl class="stats">
            ...
            <div><dt>Ann. vol (9m)</dt><dd>${pct(r.legs.v)}</dd></div>
  ...
  /* 9–1 momentum taken apart for the names that have it: raw, net of the
  ...
  function decomposition(r) {
    ...
    return `
        <div class="decomp">
          <h4>${WINDOW} momentum, taken apart</h4>
          ...
          <p class="legend">The last bar is the part of the window's move that neither the S&amp;P 900 nor the
            other ${group.toLowerCase()} explain: the middle bar takes out the name's usual sensitivity to the
            market (the score's rolling three-year beta), the last what a regression over the window on the
            market and the group accounts for. A description of the move, not a forecast.</p>
        </div>`;
  }

  /* The score, taken apart: one row per ingredient of the 9–1 window, the
     rows the settings use marked, and the score itself written out. The others
     are shown for reference. */
  function components(r, sc) {
    const s = state.score, legs = r.legs;
    const row = (label, value, klass, on) => `<tr${on ? ' class="on"' : ''}><td>${label}</td>`
      + `<td class="${klass || ''}">${value}</td></tr>`;
    const body = `
        <p class="sub">The nine months ending one month ago; the most recent month is skipped.</p>
        <table class="comp">
          <tbody>
            ${row('Return', spct(legs.r), cls(legs.r), !s.resid)}
            ${row('Net of market', spct(legs.e), cls(legs.e), s.resid)}
            ${row('Volatility (ann.)', pct(legs.v), '', s.vol && !s.resid)}
            ${row('Residual volatility', pct(legs.w), '', s.vol && s.resid)}
          </tbody>
        </table>
        <div class="blend">
          <div><b>Score</b><small>${scoreSummary()}</small></div>
          <span class="big" style="color:${tone(sc.pct)}">${FMT.value(sc.s)}</span>
        </div>
        <p class="legend">Marked rows are the ones in use: the score is ${s.resid ? 'the return net of the market'
          : 'the return'}${s.vol ? ` divided by the ${s.resid ? 'residual ' : ''}volatility` : ''}, taken as it is.
          Its rank (${FMT.rank(sc.rank)} of ${state.scored.n}) and percentile (${FMT.pct(sc.pct)}) are read across
          the whole universe. Both choices are made in Settings.</p>
        <a class="more" href="#/settings">Calculation details ›</a>`;
    return { summary: FMT.value(sc.s), body };
  }
  ...
  function seriesFor(r, bars, file) {
    ...
    const values = bars.dates.map((d, i) => {
      ...
      const sc = scoreFrom(legs);
      if (sc.s == null) return null;
      if (kind === 'value') return sc.s;
      const rank = rankIn(file.decoded[k], sc.q);
  ...
  /* A ladder ships as base64 little-endian int32. */
  function unpackLadder(b64) {
    const bin = atob(b64), out = new Int32Array(bin.length >> 2);
    for (let i = 0; i < out.length; i++) {
      out[i] = bin.charCodeAt(4 * i) | (bin.charCodeAt(4 * i + 1) << 8)
        | (bin.charCodeAt(4 * i + 2) << 16) | (bin.charCodeAt(4 * i + 3) << 24);
    }
    return out;
  }
